...
> workflow generation) — **Phase 6 now has its own detailed plan, made per-env:
> see [per-environment-registry.md](./per-environment-registry.md)**. Note
> corrected below: the `larakube-dashboard` SA is
