...
spec:
  ...
  jobTemplate:
    spec:
      template:
        spec:
          # fsGroup so www-data can write the mounted PVC on block storage (DOKS
          # etc.); harmless on k3s local-path. See base/deployment.blade.php.
          securityContext:
            fsGroup: 33
@if($config->isSystem())
...
@php($resources = $config->getResources($environment ?? 'local', 'scheduler'))
